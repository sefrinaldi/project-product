package com.sefrinaldi.productservice.controller;

import com.sefrinaldi.productservice.dto.StockRequestDto;
import com.sefrinaldi.productservice.entity.Stock;
import com.sefrinaldi.productservice.service.StockService;
import com.sefrinaldi.productservice.validator.contraint.StockProductIsNotFound;
import com.sefrinaldi.productservice.validator.contraint.StockUnderZero;
import com.sefrinaldi.productservice.validator.groups.StockControllerChecks;
import com.sefrinaldi.productservice.validator.groups.ValidateFirst;
import com.sefrinaldi.productservice.validator.groups.ValidateSecond;
import javassist.NotFoundException;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;

@RestController
@RequestMapping("/stock-service")
@Validated(value = StockControllerChecks.class)
public class StockController {

    @Autowired
    private StockService stockService;

    @PostMapping("/modify/{id}/stock")
    public Stock modifyStock(
            @PathVariable(value = "id") @StockProductIsNotFound(groups = ValidateFirst.class) Long id,
            @RequestBody @StockUnderZero(groups = ValidateSecond.class) StockRequestDto stockRequestDto) throws NotFoundException {
        return stockService.modifyStockProduct(id, stockRequestDto);
    }
}
